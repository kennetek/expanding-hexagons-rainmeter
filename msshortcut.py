...
class MSShortcut:
        ...
        with lnkPath.open('rb') as f:
            ...
            if (self._lnkFlags & 0x02):
                (linkInfoSize, linkInfoHdrSize, self._linkInfoFlags,
                 ...
                 cmnNetRelativeLinkOffset, cmnPathSuffixOffset) = struct.unpack('IIIIIII',
                                                                                content[position:position + 28])

                # check for optional offsets
                localBasePathOffsetUnicode = None
                cmnPathSuffixOffsetUnicode = None
                if linkInfoHdrSize >= 0x24:
                    (localBasePathOffsetUnicode, cmnPathSuffixOffsetUnicode) = struct.unpack('II', content[
                                                                                                   position + 28:position + 36])
    ...
    def command(self):
        if self.commandLineArgs:
            return " ".join([self.localBasePath, self.commandLineArgs])
        elif self.localBasePath:
            return self.localBasePath
        else:
            result = []
            found = False
            for thing in self.targetPath.parts:
                if not found and thing.__eq__('..'):
                    found = True
                if found and not thing.__eq__('..'):
                    result.append(thing)
            return self.targetPath.anchor + "\\".join(result)
...
class CommonNetworkRelativeLink():
    ...
    def __init__(self, scContent, linkContentPos):
        ...
        (linkSize, flags, netNameOffset,
         devNameOffset, self._networkProviderType) = struct.unpack('IIIII',
                                                                   scContent[linkContentPos:linkContentPos + 20])

        # logger.debug("netnameOffset = {}".format(netNameOffset))
        if netNameOffset > 0x014:
            (netNameOffsetUnicode, devNameOffsetUnicode) = struct.unpack('II', scContent[
                                                                               linkContentPos + 20:linkContentPos + 28])
